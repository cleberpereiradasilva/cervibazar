...
import { useCashOpenings, type CashOpening } from "@/app/hooks/useCashOpenings";
import AberturaForm from "./AberturaForm";
import AberturaTable from "./AberturaTable";

export default function AberturaClientPage() {
  const { openings, loading, saving, error, stats, create, update, remove } = useCashOpenings();
  ...
  const handleSubmit = async (input: { amount: string }) => {
    ...
    if (created.ok) {
      setFormOpen(false);
      toast.success("Abertura registrada com sucesso.");
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.ok) {
      toast.success("Abertura removida com sucesso.");
    } else if (result.error) {
      toast.error(result.error);
    }
    return result;
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-8 sm:px-6">
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-1 text-text-secondary dark:text-[#bcaec4]">
            Registre o valor de troco inicial antes de abrir o caixa.
          </p>
        </div>
      ...
      <Sheet
        ...
        title={editing ? "Editar abertura" : "Nova abertura"}
